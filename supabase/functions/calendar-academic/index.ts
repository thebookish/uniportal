...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const events: AcademicEventInput[] = Array.isArray(body) ? body : [body];
      ...
      return new Response(
        JSON.stringify({ success: true, data, message: `${data.length} academic event(s) created` }),
...
function calculateFreeTimeBlocks(weeklyCalendar: any[]): any[] {
  ...
  // For each day, find gaps between events
